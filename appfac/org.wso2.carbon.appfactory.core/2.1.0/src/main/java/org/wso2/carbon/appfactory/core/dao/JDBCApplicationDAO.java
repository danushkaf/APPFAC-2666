...
public class JDBCApplicationDAO {
        ...
        try {
            ...
            if (updatedRowCount > 0) {
                if (log.isDebugEnabled()) {
                    log.debug((new StringBuilder()).append("successfully added application.Updated ").append
            ...
            String errorMessage = "Adding application is failed for " + application.getId() + " of " + CarbonContext
            ...
            String errorMsg = "Error while adding the application " + application.getId() + " of " + CarbonContext
                    .getThreadLocalCarbonContext().getTenantDomain();
    ...
    public boolean isApplicationNameExists(String applicationName) throws AppFactoryException {
        ...
        if (nameCache.containsKey(appNameCacheKey)) {
            if (log.isDebugEnabled()) {
                log.debug("Retrieving data from the application name cache for application name : " + applicationName);
            }
            return true;
        }
        // No cache hit
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Is application name exists check for application name : " + applicationName);
            }
    ...
    private String getApplicationName(String applicationKey) throws AppFactoryException {
        ...
        try {
            ...
            applicationResult = preparedStatement.executeQuery();
            if (log.isDebugEnabled()) {
                log.debug("get application name for application key " + applicationKey);
            }
            // This means we have a result set. So need to update the cache
            if (applicationResult.next()) {
                return applicationResult.getString(SQLParameterConstants.COLUMN_NAME_APPLICATION_NAME);
            }
        } catch (SQLException e) {
            handleException("Error while getting application name " + applicationKey, e);
        } finally {
            AppFactoryDBUtil.closeResultSet(applicationResult);
            AppFactoryDBUtil.closePreparedStatement(preparedStatement);
            AppFactoryDBUtil.closeConnection(databaseConnection);
    ...
    private boolean addVersion(int applicationID, Version version, Connection databaseConnection)
        ...
        try {
            ...
            if (affectedRowCount > 0) {
                if (log.isDebugEnabled()) {
                    log.debug((new StringBuilder()).append("successfully added application  " + "version ").append
    ...
    public boolean deleteApplication(String applicationID) throws AppFactoryException {
        //If there are any calls to deleteApplication method we remove the entries from the cache
        if (log.isDebugEnabled()) {
            log.debug("Removing data from all the caches for application key : " + applicationID);
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("Successfully removed data from all the caches for application key : " + applicationID);
        }
        ...
        try {
            ...
            if (affectedRows > 0) {
                databaseConnection.commit();
                if (log.isDebugEnabled()) {
                    log.debug("Successfully deleted application : " + applicationID);
                }
                return true;
            }
    ...
    private void deleteFromBuildStatus(Connection databaseConnection, String applicationKey,
        ...
        try {
            ...
            for (Integer repositoryID : repositoryIDs) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Adding repository id " + repositoryID + " of application" + applicationKey + " for " +
                              "deletion of build status information");
                }
            }
            preparedStatement.executeBatch();
            if (log.isDebugEnabled()) {
                log.debug("Successfully deleted all application build status information of application : " +
                          applicationKey);
            }
        } catch (SQLException e) {
    ...
    private void deleteFromDeployStatus(Connection databaseConnection, String applicationKey,
        ...
        try {
            ...
            preparedStatement.executeBatch();
            if (log.isDebugEnabled()) {
                log.debug("Successfully deleted all application deploy status information of application : " +
                          applicationKey);
            }
        } catch (SQLException e) {
            // We do not rollback at this level since that is done from the calling method
    ...
    private void deleteFromRepository(Connection databaseConnection, String applicationKey, List<Integer> repositoryIDs)
        ...
        try {
            ...
            preparedStatement.executeBatch();
            if (log.isDebugEnabled()) {
                log.debug("Successfully deleted all application repository information of application : " +
                          applicationKey);
            }
        } catch (SQLException e) {
            // We do not rollback at this level since that is done from the calling method
    ...
    private void deleteFromApplicationVersion(Connection databaseConnection, String applicationKey,
        ...
        try {
            ...
            for (Integer versionID : versionIDs) {
                ...
                preparedStatement.addBatch();
                if (log.isDebugEnabled()) {
                    log.debug("Adding repository id " + versionID + " of application" + applicationKey + " for " +
                              "deletion of application version information");
                }
            }
            preparedStatement.executeBatch();
            if (log.isDebugEnabled()) {
                log.debug("Successfully deleted all application version information of application : " +
                          applicationKey);
            }
        } catch (SQLException e) {
            // We do not rollback at this level since that is done from the calling method
    ...
    private void deleteFromApplicationResources(Connection databaseConnection, String applicationKey)
        ...
        try {
            ...
            preparedStatement.execute();
            if (log.isDebugEnabled()) {
                log.debug("Successfully deleted all application resources information of application : " +
                          applicationKey);
            }
        } catch (SQLException e) {
            // We do not rollback at this level since that is done from the calling method
    ...
    public boolean addVersion(String applicationKey, Version version) throws AppFactoryException {
        ...
        try {
            ...
            try {
                ...
                // Only logging this exception since this is not the main issue. The original issue is thrown.
                log.error("Error while rolling back add version " +
                          "" + version.getId() + " of app " + applicationKey, e1);
            }
            handleException("Error while adding version " +
                            "" + version.getId() + " of app " + applicationKey, e);
        } finally {
            AppFactoryDBUtil.closeConnection(databaseConnection);
        }
    ...
    public boolean setApplicationCreationStatus(String applicationKey,
        ...
        try {
            ...
            if (affectedRows > 0) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Removing data from the application creation status cache for application key : " +
                              applicationKey);
                }
    ...
    public Constants.ApplicationCreationStatus getApplicationCreationStatus(String applicationKey)
        ...
        if (applicationCreationStatusCache.containsKey(applicationCreationStatusCacheKey)) {
            if (log.isDebugEnabled()) {
                log.debug("Retrieving data from the application creation status cache for application key : " +
                          applicationKey);
            }
    ...
    public Map<String, Constants.ApplicationCreationStatus> getApplicationCreationStatusByKeys(String[] appKeyArray)
        ...
        if (log.isDebugEnabled()) {
            log.debug("Retrieving data from the application creation status cache for multiple application keys");
        }
    ...
    public boolean updateLastBuildStatus(String applicationKey, String version, boolean isForked, String username,
        ...
        try {
            ...
            if (affectedRows > 0) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Removing data from the build status cache for application key : " + applicationKey);
                }
    ...
    public BuildStatus getBuildStatus(String applicationKey, String version, boolean isForked, String username)
        ...
        if (buildStatusCache.containsKey(buildStatusCacheKey)) {
            if (log.isDebugEnabled()) {
                log.debug("Retrieving data from the build status cache for application key : " + applicationKey);
            }
        ...
        try {
            databaseConnection = AppFactoryDBUtil.getConnection();
            getAppLastBuildPreparedStatement = databaseConnection.prepareStatement(SQLConstants
                                                                                           .GET_APPLICATION_LAST_BUILD_SQL);
            int repositoryID = getRepositoryID(applicationKey, isForked, username, version, databaseConnection);
            ...
            if (buildResultSet.next()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("There is no result for query  get build status app key=" +
    ...
    public boolean updateLastDeployStatus(String applicationKey, String version, String environment, boolean isForked,
        ...
        try {
            ...
            if (affectedRows > 0) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Removing data from the deployment status cache for application key : " + applicationKey);
                }
    ...
    public boolean updateLastDeployedBuildID(String applicationKey, String version, String environment,
        ...
        try {
            ...
            try {
                ...
                // Only logging this exception since this is not the main issue. The original issue is thrown.
                log.error("Error while rolling back update deployed build id for version " +
                          "" + version + " of " +
                          "application " + applicationKey, e1);
            }
    ...
    public DeployStatus getDeployStatus(String applicationKey, String version, String environment, boolean isForked,
        ...
        if (deployCache.containsKey(deployCacheKey)) {
            if (log.isDebugEnabled()) {
                log.debug("Retrieving data from the deployment status cache for application key : " + applicationKey);
            }
    ...
    int getApplicationID(String applicationKey, Connection databaseConnection) throws AppFactoryException {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Getting Application ID " + applicationID);
            }
        } catch (SQLException e) {
    ...
    private int getVersionID(int applicationID, String version, Connection databaseConnection)
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Getting Version ID " + versionID);
            }
        } catch (SQLException e) {
    ...
    private int getRepositoryID(int versionID, boolean isFork, String userID, Connection databaseConnection)
        ...
        try {
            if (isFork) {
                repositoryIDPreparedStatement = databaseConnection.prepareStatement(SQLConstants
                                                                                            .GET_FORKED_APPLICATION_REPOSITORY_ID_SQL);
                repositoryIDPreparedStatement.setInt(1, versionID);
                repositoryIDPreparedStatement.setString(2, userID);
            } else {
                repositoryIDPreparedStatement = databaseConnection.prepareStatement(SQLConstants
                                                                                            .GET_APPLICATION_REPOSITORY_ID_SQL);
                repositoryIDPreparedStatement.setInt(1, versionID);
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug("Getting Repository ID " + repositoryID);
            }
        } catch (SQLException e) {
     ...
     * @return {@link org.wso2.carbon.appfactory.core.dto.CartridgeCluster}
    ...
    private void removeApplicationDataFromAllCaches(String applicationKey) throws AppFactoryException {
        ...
        if (applicationName != null) {
            ...
            applicationNameCache.remove(applicationName);
        }
