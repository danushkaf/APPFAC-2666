...
public class JenkinsApplicationEventsListener extends ApplicationEventsHandler {
    ...
    public void onCreation(Application application, String userName, String tenantDomain, boolean isUploadableAppType)
        ...
        if (!AppFactoryUtil.isBuildServerRequiredProject(application.getType())) {
            return;
        }
        ...
        try {
            String infoMsg = "Jenkins space for " + application.getId() + " created.";
            String infoMessageDesc =
                    "Initial provisioning of Jenkins build system for application:" +
                            application.getId() + " is successfully completed.";
            EventNotifier.getInstance().notify(EventBuilderUtil.buildApplicationCreationEvent(infoMsg,
                    infoMessageDesc,Event.Category.INFO));
    ...
    public void onDeletion(Application application, String userName, String tenantDomain)
        ...
        applicationDeployer.undeployAllArtifactsOfAppFromDepSyncGitRepo(application.getId(),
                application.getType(),
                ProjectUtils.getVersions(application.getId(),
                        tenantDomain));
    }
    ...
    public void onFork(Application application, String userName, String tenantDomain,
        ...
        if (!AppFactoryUtil.isBuildServerRequiredProject(application.getType()) || (perDeveloperBuild == null || perDeveloperBuild.equals("false"))) {
            return;
        }
