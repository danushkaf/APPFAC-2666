...
public class AppFactoryApplicationEventListener extends ApplicationEventsHandler {
    ...
    public void onCreation(Application application, String userName, String tenantDomain, boolean isUploadableAppType) throws AppFactoryException {
        try {
            ...
            String infoMessage = "Issue tracker provisioning for application:" + application.getId() + " is successfully completed.";
            log.info(infoMessage);
            try {
                EventNotifier.getInstance().notify(EventBuilderUtil.buildApplicationCreationEvent(
                        "Issue tracker space created for " + application.getId() + ".",
                        infoMessage, Event.Category.INFO));
