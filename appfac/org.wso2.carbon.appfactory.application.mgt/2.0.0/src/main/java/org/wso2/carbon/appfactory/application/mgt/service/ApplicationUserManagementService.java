...
public class ApplicationUserManagementService {
    ...
    public boolean addUsersToApplication(String userNames[], String applicationKey)
        ...
        try {
            ...
            //Notfiy to App wall
            try {
                String infoMessage = "New users invited to " + applicationKey;
                String user = "user";
                String users = "users";
                String description = null;
                String userNameStr = "";
                for(String userName : userNames){
                    userNameStr = userNameStr.concat(", ").concat(userName);
                }
                if(userNames.length ==  1){
                    description = "New " + user + " " + userNameStr + " is successfully added to the application " + applicationKey;
                } else if (userNames.length > 1) {
                    description = "New " + users + " " + userNameStr + " are successfully added to the application " + applicationKey;
                }
                EventNotifier.getInstance().notify(EventBuilderUtil.buildUserAdditionToAppEvent(applicationKey, infoMessage,
                        description, Event.Category.INFO));
            ...
            try {
                String errorMessage = "User addition to " + applicationKey + " failed.";
                String user = "user";
                String users = "users";
                String description = null;
                String userNameStr = "";
                ...
                if(userNames.length ==  1){
                    description = "New " + user + " " + userNameStr + " is addition to the application " + applicationKey + " failed.";
                } else if (userNames.length > 1) {
                    description = "New " + users + " " + userNameStr + " are addition to the application " + applicationKey + " failed.";
                }
                EventNotifier.getInstance().notify(EventBuilderUtil.buildUserAdditionToAppEvent(applicationKey, errorMessage,
                        description, Event.Category.ERROR));
            } catch (AppFactoryEventException exception) {
            ...
            try {
                String errorMessage = "New user addition to " + applicationKey + " failed.";
                String user = "user";
                String users = "users";
                String description = null;
                String userNameStr = "";
                ...
                EventNotifier.getInstance().notify(EventBuilderUtil.buildUserAdditionToAppEvent(applicationKey,
                        errorMessage, description, Event.Category.ERROR));
            } catch (AppFactoryEventException exception) {
                log.error("Failed to notify the failure of user addition to app event ", exception);
                // do not throw again.
            }
