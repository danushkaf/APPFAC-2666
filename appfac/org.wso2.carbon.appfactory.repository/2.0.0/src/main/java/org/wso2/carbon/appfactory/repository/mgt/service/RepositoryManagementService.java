...
public class RepositoryManagementService extends AbstractAdmin {
    ...
    public void branch(String appId, String currentVersion, String targetVersion,
        ...
        try {
            ...
            repositoryManager.branch(appId, getRepositoryType(appId, tenantDomain), currentVersion,
                                     targetVersion, currentRevision, tenantDomain);

            try {
                String infoMessage =
                                     "Branch " + targetVersion +
                                             " created successfully for " + appId;
                String infoMsgDesc =  "Branch " + targetVersion + " was successfully created for application " + appId;
                EventNotifier.getInstance()
                             .notify(EventBuilderUtil.buildBranchCreationEvent(appId, infoMessage,
                                                                               infoMessage,
                                                                               Event.Category.INFO));
            ...
            String messageBody = "Error occurred while creating the branch " + targetVersion + "due to " + e.getMessage();
            try {
                String errorMessage = "Error while creating branch " + targetVersion + " for " + appId;
                EventNotifier.getInstance().notify(EventBuilderUtil.buildBranchCreationEvent(appId,
                        errorMessage,messageBody,Event.Category.ERROR));
            ...
            throw new RepositoryMgtException(messageBody, e);
        }
    ...
    public void createForkBranch(String applicationKey, String userName, String type, String version)
        ...
        try {
            ...
            String msgBody =
                             "Error while forking branch for application " + applicationKey +
                                     "due to " + e.getMessage();
            try {
                String errorMessage =
                                      "Error while adding version " + version +
                                              " for the forked repo of " +
                                              applicationKey;
                EventNotifier.getInstance()
                             .notify(EventBuilderUtil.buildBranchForkingEvent(applicationKey,
                                                                              errorMessage,
                                                                              msgBody,
            ...
            log.error(msgBody, e);
            throw new RepositoryMgtException(msgBody, e);
        }
    }
