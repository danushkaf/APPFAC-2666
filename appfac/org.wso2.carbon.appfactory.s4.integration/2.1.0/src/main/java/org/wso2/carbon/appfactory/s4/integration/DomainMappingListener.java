...
public class DomainMappingListener extends ApplicationEventsHandler {
    ...
    public void onDeletion(Application application, String userName, String tenantDomain) throws AppFactoryException {
        try {
            if (AppFactoryCoreUtil.isDomainMappingAllowedAppType(application.getType())) {
                // remove custom url
                ServiceReferenceHolder.getInstance().getDomainMappingManagementService().removeDomainMappingFromApplication(
                        ServiceReferenceHolder.getInstance().getAppFactoryConfiguration().getFirstProperty("FineGrainedDomainMappingAllowedStage"),
                        application.getId(), null, true);
            ...
            log.error("Error while removing default and custom urls for application:" + application.getId() + ", tenant:" + tenantDomain);
        }
    }
