...
public class DomainMappingManagementService {
    ...
    public void addSubscriptionDomain(String stage, String domain, String appKey, String version,
        ...
        String addSubscriptionDomainEndPoint =
                DomainMappingUtils.getAddDomainEndPoint(stage);

        synchronized (domain.intern()) {

            // Check domain availability
            boolean domainAvailable = isDomainAvailable(stage, domain);
            if (!domainAvailable) {
    ...
    public void remapDomainToContext(String stage, String domain, String appKey,
        ...
        removeSubscriptionDomain(stage, domain);
    ...
    public void remapContextToDomain(String stage, String newDomain,
        ...
        try {
            ...
            if (StringUtils.isNotBlank(previousDomain)) {
                try {
                    removeSubscriptionDomain(stage, previousDomain);
    ...
    public void removeDomainMappingFromApplication(String stage, String appKey, String version,
        ...
        try {
            if (isCustomDomain) {
                String customDomain = DomainMappingUtils.getCustomDomain(appKey);
                if (StringUtils.isNotBlank(customDomain)) {
                    removeSubscriptionDomain(stage, customDomain);
                    DomainMappingUtils.updateCustomUrlMetadata(appKey, DomainMappingUtils.UNDEFINED_URL_RXT_VALUE, DomainMappingUtils.UNVERIFIED_VERIFICATION_CODE);
                    // NOTE: need not update the appversion rxt as removed since default prod url has not removed
                    log.info("Successfully removed custom domain: " + customDomain + " from application id:" + appKey);
                    notifyAppWall(appKey, AF_APPWALL_REMOVE_SUCCESS_MSG, String.format(AF_APPWALL_URL, customDomain), Event.Category.INFO);
                }
            } else {
                String defaultDomain = DomainMappingUtils.getDefaultDomain(appKey);
                if (StringUtils.isNotBlank(defaultDomain)) {
                    removeSubscriptionDomain(stage, defaultDomain);
                    DomainMappingUtils.updateApplicationMetaDataMappedDomain(appKey, DomainMappingUtils.UNDEFINED_URL_RXT_VALUE);
                    if (StringUtils.isNotBlank(version)) {   // update the appversion rxt as removed
                        DomainMappingUtils.updateVersionMetadataWithMappedDomain(appKey, version, DomainMappingUtils.UNDEFINED_URL_RXT_VALUE);
                    }
                    log.info("Successfully removed default domain: " + defaultDomain + " from application id:" + appKey);
                    notifyAppWall(appKey, AF_APPWALL_REMOVE_SUCCESS_MSG, String.format(AF_APPWALL_URL, defaultDomain), Event.Category.INFO);
                }
            }
        } catch (AppFactoryException e) {
            log.error("Error occurred while removing domain mapping for appication id:"+appKey+" stage: "+stage,e);
            notifyAppWall(appKey, AF_APPWALL_ERROR_MSG, "", Event.Category.ERROR);
            throw new AppFactoryException(DomainMappingUtils.AF_ERROR_REMOVE_DOMAIN_MSG);
        }
    }
    ...
    public void removeSubscriptionDomain(String stage, String domain) throws AppFactoryException {
        ...
        String removeSubscriptionDomainEndPoint =
                DomainMappingUtils.getRemoveDomainEndPoint(stage, domain);
        ...
        try {
            ...
            log.error("Error occurred removing domain mapping for domain: " + domain + " in stage :" + stage, e);
        ...
        if (deleteResponse.statusCode == HttpStatus.SC_OK) {
            ...
            log.info("Successfully removed domain :" + domain);
            if (log.isDebugEnabled()) {
                log.debug("Stratos response status: " + deleteResponse.statusCode + " Stratos Response message: " + deleteResponse.getResponse());
            }
        } else {
            log.error("Error occurred removing domain mapping for domain: " + domain + " in stage :" + stage+
                      " Stratos response status: " + deleteResponse.statusCode + " Stratos Response message: " + deleteResponse.getResponse());
            throw new AppFactoryException(String.format(DomainMappingUtils.AF_ERROR_GENARIC_MSG, domain));
        }
     ...
     */
    private boolean isDomainAvailable(String stage, String domain) throws AppFactoryException {
        boolean isAvailable;
        String validateSubscriptionDomainEndPoint =
                DomainMappingUtils.getDomainAvailableEndPoint(stage, domain);
